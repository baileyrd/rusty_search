...
| [`rusty-search-sqlite-fts5`](crates/rusty-search-sqlite-fts5) | A `SearchBackend` backed by SQLite's [FTS5](https://www.sqlite.org/fts5.html) virtual table module - genuinely embedded like `rusty-search-tantivy`, but via SQL rather than an inverted-index library, so `Query::Term`/`Query::Range`/`Sort::Field` all run as plain SQL on every field type with no fast-field opt-in needed. Runs in-process (`SqliteFts5Backend::in_memory()`) or persists to disk (`SqliteFts5Backend::on_disk(dir)`). See ADR-0008. |
...
| [`rusty-search`](crates/rusty-search) | The facade crate applications depend on. Re-exports `rusty-search-core` plus each backend behind a feature flag (`memory`, `tantivy`, `sqlite-fts5`, `elasticsearch`, `opensearch`, `meilisearch`, `solr`, `algolia`, `azure-search`), mirroring how `sqlx` gates its database drivers. |
...
the concrete engine at runtime - in-memory in tests, Tantivy, SQLite FTS5,
Elasticsearch, OpenSearch, Meilisearch, Solr, a hosted Algolia application,
or a hosted Azure AI Search service in production - exactly as you'd swap a
...
# add `,sqlite-fts5`/`,elasticsearch`/`,opensearch`/`,meilisearch`/`,solr`/`,algolia`/
...
This crate is a young, from-scratch project. The core interface and nine
backends (in-memory, Tantivy, SQLite FTS5, Elasticsearch, OpenSearch,
Meilisearch, Solr, Algolia, Azure AI Search) are implemented and tested;
see each crate's module-level docs for known limitations (e.g.
`rusty-search-tantivy`'s sort support, `rusty-search-sqlite-fts5`'s
single-`Query::Match`-clause restriction, `rusty-search-meilisearch`'s
query restrictions, `rusty-search-algolia`'s lack of a native per-query
field sort or relevance score, `rusty-search-azure-search`'s fast-field-like
sortable requirement). Contributions adding backends for other engines are
welcome.

`SearchRequest` also carries an optional `vector: Option<VectorQuery>`
(fused with the lexical `query` by backends that support it, checked via
`SearchBackend::supports_vector_search()`), settling the `Query` DSL's
vector/hybrid design question - see ADR-0008. No backend implements it
yet; `rusty-search-sqlite-fts5` (via `sqlite-vec`) is the natural first
one to.
...
- **`sqlite-vec` support in `rusty-search-sqlite-fts5`**, to actually
  implement hybrid search now that ADR-0008 has settled how it plugs into
  the `Query` DSL (`SearchRequest::vector`, not a `Query` variant) - the
  concrete next step now that the design question that used to block this
  whole category is answered, rather than an open-ended "vector/hybrid
  search" item.
