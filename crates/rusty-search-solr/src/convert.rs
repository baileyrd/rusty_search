...
/// configset already defines - and keeps it out of `Document::fields` on
/// the way back.
pub const ID_FIELD: &str = "id";

/// Converts a core [`Document`] into the JSON object Solr's update handler
...
/// Converts a Solr document (as returned by `/select`) back into a core
/// [`Document`], pulling `"id"` out into [`Document::id`] and dropping
/// Solr's own `"score"` pseudo-field (surfaced separately as `Hit::score`).
...
mod tests {
    ...
    #[test]
    fn json_to_document_pulls_id_and_score_out_of_fields() {
        let value = serde_json::json!({ "id": "1", "title": "hello", "score": 1.5 });
